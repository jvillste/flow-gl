...
(deflayout HorizontalStack [layoutables]
             ...
             (loop [layouted-layoutables []
               ...
               (if (seq layoutables)
                 (let [width (layoutable/preferred-width (first layoutables))]
                   (recur (conj layouted-layoutables (set-dimensions-and-layout (first layoutables) x 0    width height))
                          (+ x width)
                          (rest layoutables)))
                 layouted-layoutables)))))
...
(deflayout SizeGroupMember [size-group mode layoutable]
